...
ast_t *parse_expression_1(token_t **t_orig, ast_t *lhs, int min_prec) {
  token_t *t = *t_orig;
  token_t *lookahead = t->next;
  if (lookahead->type == semicolon)
    return parse_primary(t);
  for (; precedence(lookahead->type) >= min_prec;) {
    token_t *op = lookahead;
    t = t->next->next;
    ast_t *rhs = parse_primary(t);
    lookahead = t->next;
    if (lookahead->type != semicolon) {
      for (; precedence(lookahead->type) >= precedence(op->type);) {
        int is_higher = (precedence(lookahead->type) > precedence(op->type));
        rhs = parse_expression_1(&t, rhs, precedence(op->type) + is_higher);
        lookahead = t;
        if (lookahead->type == semicolon) {
          break;
        }
    ...
    lhs->right = rhs;
    lhs->operator= type_to_operator_char(op->type);
    if (lookahead->type == semicolon) {
      t = lookahead;
      break;
    }
